...
import PlantIcon from "../Svg/Icons/Logo";
import { PlantProps, FallingPlantsProps } from "./types";

const plantFall = keyframes`
  0% {
    opacity: 1;
...
const FallingPlants: React.FC<FallingPlantsProps> = ({
  count = 30,
  size = 32,
  iterations = Infinity,
  duration = 10,
}) => {
  const plants = [...Array(count)].map((_, index) => (
    <Plant key={String(index)} position={Math.random() * 100} iterations={iterations} duration={duration}>
      <PlantIcon width={size} height={size} />
    </Plant>
  ));
